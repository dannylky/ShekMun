...
function Write-ErrorLog {
    ...
    try {
        Add-Content -Path $script:errorLog -Value "$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')  $Message" -Encoding UTF8 -ErrorAction Stop
    } catch {}
}
...
function Save-Snapshot {
    ...
    for ($i = 0; $i -lt 3; $i++) {
        try {
            ...
            if ($i -lt 2) { Start-Sleep -Milliseconds 250; continue }
            Write-ErrorLog "snapshot write failed (file locked?): $err"
        }
    }
}
...
function Write-LogLine {
    ...
    for ($i = 0; $i -lt 3; $i++) {
        try {
            ...
            $err = $_.Exception.Message
            if ($i -lt 2) { Start-Sleep -Milliseconds 250; continue }
            Write-ErrorLog "log write failed (file locked?): $File ($err)"
        }
    }
}
...
if ($OneShot) {
    ...
    try { Write-RunReport -Label 'One-shot' }
    ...
    while ($true) {
        ...
        foreach ($job in $jobs) {
            ...
            try {
                ...
                Write-ErrorLog "check failed for $($job.room)/$($job.name) ($($job.ip)): $($_.Exception.Message)"
            }
